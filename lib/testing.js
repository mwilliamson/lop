...
var assertIsSuccessWithValue = exports.assertIsSuccessWithValue = function(test, result, value, source, remaining) {
    assertIsSuccess(test, result);
    assertThat(test, result.value(), duck.is(value));
    if (source) {
        test.deepEqual(result.source(), source);
    }
    if (!remaining) {
        remaining = [tokeniser.tokens.end()];
    }
    assertRemaining(test, result, remaining);
};
