    ...
    return function(input) {
        return lazyIterators
            ...
            .first() || results.failure([errors.error({
                expected: name,
                actual: describeToken(input.head()),
                location: input.head().source
            })], input);
    };
};
...
exports.sequence = function() {
    ...
    rule.map = function(func) {
        var captures = parsers.filter(isCapturedRule);
        return exports.then(
            rule,
            exports.sequence.applyValues.apply(this, [func].concat(captures))
        );
    };
    
    function isCapturedRule(subRule) {
        return !!subRule.captureName;
    }
