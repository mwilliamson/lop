...
exports.sequence = function() {
    ...
    return function(input) {
        return _.foldl(parsers, function(memo, parser) {
            ...
            if (subResult.isCut()) {
                ...
                return {result: results.success(values, subResult.remaining()), hasCut: hasCut};
            } else if (hasCut) {
                return {result: results.error(subResult, subResult.remaining()), hasCut: hasCut};
            } else {
...
exports.zeroOrMoreWithSeparator = function(rule, separator) {
    return function(input) {
        var result = rule(input);
        if (result.isSuccess()) {
            var mainRule = exports.capture(rule, "main");
            var remainingRule = repeated(exports.then(
            ...
            if (remainingResult.isSuccess()) {
                ...
                return results.success([result.value()], result.remaining());
            }
        } else {
...
var repeated = function(rule) {
    return function(input) {
        ...
        return results.success(values, result.remaining());
    };
};
